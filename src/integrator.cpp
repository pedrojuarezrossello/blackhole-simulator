...
void schwarzschild_integrator::_update_directions(MFLOAT * new_radii_ps, size_t idx) {
...
MFLOAT schwarzschild_integrator::_rhs_radius_ode_squared(MFLOAT total_energy, MFLOAT arg_for_potential, size_t idx) {
	// a^2 - b^2 = a*a - b^2
	MFLOAT potential = _potential_energy(arg_for_potential, idx);
	return FMSUB(total_energy, total_energy, MUL(potential, potential));
}

MFLOAT schwarzschild_integrator::_next_step_radius(MFLOAT prev_radius, float step, size_t idx) {
...
MFLOAT schwarzschild_integrator::_next_step_phi(MFLOAT radius, MFLOAT prev_phi, float step, size_t idx) {
...
MFLOAT schwarzschild_integrator::_potential_energy(MFLOAT radius, size_t idx) {
...
std::vector<float> schwarzschild_integrator::_initialise_energies(const std::vector<float> & radii) {
...
float schwarzschild_integrator::_potential_energy_scalar(float radius, size_t idx) {
	return (1.0f - 2.0f * bm_scalar / radius) * (1.0f + ((angular_momenta[idx] * angular_momenta[idx]) / (radius * radius)));
}

float schwarzschild_integrator::_rhs_ode_squared_scalar(float arg_for_potential, size_t idx) {
...
float schwarzschild_integrator::_next_step_radius_scalar(float step, size_t idx) {
	//k_1 = f(y_n)
...
void schwarzschild_integrator::_update_directions_scalar(size_t idx) {
...
float schwarzschild_integrator::_next_step_phi_scalar(float step, size_t idx) {
	return phis[idx] + ((step * angular_momenta[idx]) / (radii[idx] * radii[idx]));
}

schwarzschild_integrator::schwarzschild_integrator(float _black_hole_mass, initial_particle_data<schwarzschild> _initial_data)
...
MFLOAT schwarzschild_integrator::next_radius(float step, size_t idx) {
...
MFLOAT schwarzschild_integrator::next_phi(float step, size_t idx) {
...
void schwarzschild_integrator::send_data(message& data) {
	size_t N = radii.size();
	size_t i = 0;
	for (; i + subproblem_size <= N; i += subproblem_size) {
...
void schwarzschild_integrator::send_initial_data(message& data) {
	size_t N = radii.size();
	size_t i = 0;
	for (; i + subproblem_size <= N; i += subproblem_size) {
...
void schwarzschild_integrator::rock_n_roll() {
	message data(radii.size());
	send_initial_data(data);
	for (;;)
		send_data(data);
}

std::vector<float> kerr_integrator::_compute_kappa() {
...
MFLOAT kerr_integrator::_compute_D(MFLOAT radius) {
...
MFLOAT kerr_integrator::_compute_S_inv(MFLOAT radius, MFLOAT cos_theta_ps) {
...
MFLOAT kerr_integrator::_compute_r_dot(MFLOAT D, MFLOAT S_inv, MFLOAT p_r) {
	return MUL(D, MUL(S_inv, p_r));
}

MFLOAT kerr_integrator::_compute_theta_dot(MFLOAT S_inv, MFLOAT p_theta) {
	return MUL(S_inv, p_theta);
}

MFLOAT kerr_integrator::_compute_phi_dot(MFLOAT radius, MFLOAT sin_theta_ps, MFLOAT D, MFLOAT E, MFLOAT L, MFLOAT S_inv) {
...
MFLOAT kerr_integrator::_compute_p_r_dot(MFLOAT radius, MFLOAT D, MFLOAT S_inv, MFLOAT E, MFLOAT k, MFLOAT p_r, MFLOAT L) {
	MFLOAT radius_squared_ps = MUL(radius, radius);
...
MFLOAT kerr_integrator::_compute_p_theta_dot(MFLOAT sin_theta_ps, MFLOAT cos_theta_ps, MFLOAT S_inv, MFLOAT L, MFLOAT E) {
...
kerr_integrator::geodesic_data kerr_integrator::_next_step_geodesic(size_t idx) {

	MFLOAT above_error_mask = SETZERO;

	MFLOAT radius_ps = SETZERO;
	MFLOAT theta_ps = SETZERO;
	MFLOAT p_r_ps = SETZERO;
	MFLOAT p_theta_ps = SETZERO;
	MFLOAT phi_ps = SETZERO;
	MFLOAT step_ps = LOAD(&step[idx]);

	do {
		// Load data from last step (y_n in the RK4 notation)
		radius_ps = LOAD(&radii[idx]);
		theta_ps = LOAD(&thetas[idx]);
		p_r_ps = LOAD(&p_r[idx]);
		p_theta_ps = LOAD(&p_theta[idx]);
		phi_ps = LOAD(&phis[idx]);
		//	step_ps = LOAD(&step[idx]);
		...
		//K1 -------------------------------------------------------------------
		MFLOAT sin_theta_ps = SIN(theta_ps);
		MFLOAT cos_theta_ps = COS(theta_ps);

		MFLOAT D_ps = _compute_D(radius_ps);
		MFLOAT S_inv_ps = _compute_S_inv(radius_ps, cos_theta_ps);

		MFLOAT radius_k1_ps = MUL(step_ps, _compute_r_dot(D_ps, S_inv_ps, p_r_ps));
		MFLOAT phi_k1_ps = MUL(step_ps, _compute_phi_dot(radius_ps, sin_theta_ps, D_ps, energy_ps, angular_momentum_ps, S_inv_ps));
		...
		MFLOAT p_theta_k1_ps = MUL(step_ps, _compute_p_theta_dot(sin_theta_ps, cos_theta_ps, S_inv_ps, angular_momentum_ps, energy_ps));
		...
		sin_theta_ps = SIN(adjusted_theta_ps);
		cos_theta_ps = COS(adjusted_theta_ps);

		// Recalculate S and D
		D_ps = _compute_D(adjusted_radius_ps);
		S_inv_ps = _compute_S_inv(adjusted_radius_ps, cos_theta_ps);
		MFLOAT radius_k2_ps = MUL(step_ps, _compute_r_dot(D_ps, S_inv_ps, adjusted_p_r_ps));
		MFLOAT phi_k2_ps = MUL(step_ps, _compute_phi_dot(adjusted_radius_ps, sin_theta_ps, D_ps, energy_ps, angular_momentum_ps, S_inv_ps));
		...
		MFLOAT p_theta_k2_ps = MUL(step_ps, _compute_p_theta_dot(sin_theta_ps, cos_theta_ps, S_inv_ps, angular_momentum_ps, energy_ps));
		...
		MFLOAT phi_k3_ps = MUL(step_ps, _compute_phi_dot(adjusted_radius_ps, sin_theta_ps, D_ps, energy_ps, angular_momentum_ps, S_inv_ps));
		...
		MFLOAT p_theta_k3_ps = MUL(step_ps, _compute_p_theta_dot(sin_theta_ps, cos_theta_ps, S_inv_ps, angular_momentum_ps, energy_ps));
		...
		MFLOAT phi_k4_ps = MUL(step_ps, _compute_phi_dot(adjusted_radius_ps, sin_theta_ps, D_ps, energy_ps, angular_momentum_ps, S_inv_ps));
		...
		MFLOAT p_theta_k4_ps = MUL(step_ps, _compute_p_theta_dot(sin_theta_ps, cos_theta_ps, S_inv_ps, angular_momentum_ps, energy_ps));
		...
		MFLOAT phi_k5_ps = MUL(step_ps, _compute_phi_dot(adjusted_radius_ps, sin_theta_ps, D_ps, energy_ps, angular_momentum_ps, S_inv_ps));
		...
		MFLOAT p_theta_k5_ps = MUL(step_ps, _compute_p_theta_dot(sin_theta_ps, cos_theta_ps, S_inv_ps, angular_momentum_ps, energy_ps));
		...
		MFLOAT phi_k6_ps = MUL(step_ps, _compute_phi_dot(adjusted_radius_ps, sin_theta_ps, D_ps, energy_ps, angular_momentum_ps, S_inv_ps));
		...
		MFLOAT p_theta_k6_ps = MUL(step_ps, _compute_p_theta_dot(sin_theta_ps, cos_theta_ps, S_inv_ps, angular_momentum_ps, energy_ps));
	...
	} while (!TESTZ(above_error_mask, above_error_mask));
...
kerr_integrator::geodesic_data_scalar kerr_integrator::_next_step_geodesic_scalar(size_t idx) {
...
kerr_integrator::kerr_integrator(float _a, initial_particle_data<kerr> _initial_data)
...
kerr_integrator::geodesic_data kerr_integrator::next_geodesic(size_t idx) {
...
void kerr_integrator::send_data(message& data) {
	size_t N = radii.size();
	size_t i = 0;
...
void kerr_integrator::send_initial_data(message& data) {
	size_t N = radii.size();
	size_t i = 0;
	for (; i + subproblem_size <= N; i += subproblem_size) {
		auto r = LOAD(&radii[i]);
...
// Define buffers HERE and reuse them
void kerr_integrator::rock_n_roll() {
	message data(radii.size());
	send_initial_data(data);
	for (;;)
		send_data(data);
