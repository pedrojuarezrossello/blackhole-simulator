...
extern fixed_sized_message_queue<message, true> data_queue;
...
struct message {
	...
	void send() {
		...
		data_queue.push(std::move(xs), std::move(ys), std::move(zs), std::move(radii), std::move(states));

		// Revalidate the vectors for the next batch of data
		xs.reserve(size);
		xs.resize(size);
		ys.reserve(size);
		ys.resize(size);
	...
	void set(std::vector<float> _xs, std::vector<float> _ys, std::vector<float> _zs, std::vector<float> _radii, std::vector<particle_state> _states) {
		xs = std::move(_xs);
		ys = std::move(_ys);
		zs = std::move(_zs);
